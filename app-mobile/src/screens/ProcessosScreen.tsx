...
const FORM_MAX = 440; // altura máxima visível do formulário (faz aparecer a barra de rolagem)
...
export default function ProcessosScreen() {
  ...
  // animação: 0 = expandido, 1 = recolhido
  const anim = useRef(new Animated.Value(0)).current;
  const maxHeight = anim.interpolate({
    inputRange: [0, 1],
    outputRange: [FORM_MAX, 0],
  });
  ...
  async function criar(data: ProcessoForm) {
    ...
    await scheduleLocal(
      'Processo — Atenção',
      `Processo ${data.processNumber} está perto de vencer.`,
      new Date(Date.now() + 4 * 24 * 60 * 60 * 1000)
    );
    await scheduleLocal(
      'Processo — Vencido',
      `Processo ${data.processNumber} venceu.`,
  ...
  const ListHeader = (
    <View>
      ...
      {/* Formulário com altura animada e ScrollView interno (com barra de rolagem) */}
      <Animated.View style={[s.animatedWrap, { maxHeight }]}>
        <ScrollView
          style={{ maxHeight: FORM_MAX }}
          ...
          <Card>
            <Input
              ...
              onChangeText={(t: string) =>
                setValue('processNumber', toUpperAlnum(t), { shouldValidate: true })
              }
            ...
            <Input
              ...
              placeholder="AHL"
              onChangeText={(t: string) =>
                setValue('tipo', toUpperAlnum(t) as any, { shouldValidate: true })
              }
            ...
            {tipo === 'AHL' && (
              <>
                <Input
                  label="Cliente"
                  onChangeText={(t: string) =>
                    setValue('cliente', t, { shouldValidate: true })
                  }
                ...
                <Input
                  label="PNR"
                  onChangeText={(t: string) =>
                    setValue('pnr', t, { shouldValidate: true })
                  }
            ...
            {(tipo === 'AHL' || tipo === 'OHD') && (
              <Input
                label="Descrição da bag (cor/material/padrão/tamanho/bagtag)"
                onChangeText={(t: string) =>
                  setValue('bag', t, { shouldValidate: true })
                }
            ...
            {tipo === 'DPR' && (
              <>
                <Input
                  label="Descrever dano"
                  onChangeText={(t: string) =>
                    setValue('dano', t, { shouldValidate: true })
                  }
                  error={errors.dano?.message}
                />

                <Input
                  label="Solução (Conserto|Mala nova|Voucher|Não há tratativas)"
                  onChangeText={(t: string) =>
                    setValue('solucao', t as any, { shouldValidate: true })
                  }
  ...
  return (
    <View style={s.container}>
      <FlatList
        ...
        nestedScrollEnabled               // permite rolagem do ScrollView interno
        renderItem={({ item }) => (
          <Card>
...
const s = StyleSheet.create({
  ...
  title: { color: colors.text, fontSize: 22, fontWeight: '800', marginTop: 12, marginBottom: 10 },
  ...
  // animated wrap
  animatedWrap: { overflow: 'hidden', marginBottom: 10 },
